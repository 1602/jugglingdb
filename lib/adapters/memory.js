...
function applyFilter(filter) {
    ...
    return function (obj) {
        ...
        keys.forEach(function (key) {
            if ( key === 'or' ) {
                pass = testOr(filter.where[key], obj);
            }else if (!test(filter.where[key], obj[key])) {
                pass = false;
            }
        });
        return pass;
    ...
    function testOr(or, obj) {
        if (or.constructor.name !== 'Array') return undefined;
        for (var i in or) {
            var pass = true;
            for (var key in or[i]) {
                if (key === 'or') {
                    if (!testOr(or[i][key], obj)) pass = false;
                }else if (!test(or[i][key], obj[key])) {
                    pass = false;
                }
            }
            if (pass) return true;
    ...
    if (where) {
        data = data.filter(function (id) {
            ...
            Object.keys(where).forEach(function (key) {
                if (key === 'or') {
                    ok = testOr(where[key], JSON.parse(cache[id]));
                }else if (JSON.parse(cache[id])[key] != where[key]) {
                    ok = false;
                }
            });
            return ok;
