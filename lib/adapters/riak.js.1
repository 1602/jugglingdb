...
Riak.prototype.exists = function(model, id, callback) {
    this.client.exists(this.table(model), id, function(err, exists) {
        if (callback) {
            callback(err, exists);
        }
    });
...
Riak.prototype.find = function find(model, id, callback) {
    this.client.get(this.table(model), id, function(err, data) {
        if (data && data.id) {
            data.id = id;
        } else {
            data = null;
...
Riak.prototype.all = function all(model, filter, callback) {
    ...
    this.client.getAll(this.table(model), function(err, result) {
...
Riak.prototype.destroyAll = function destroyAll(model, callback) {
    ...
    this.all(this.table(model), {}, function(err, recs) {
        ...
        function removeOne(error) {
            err = err || error;
            var rec = recs.pop();
            if (!rec) return callback(err && String(err.statusCode) !== '404' ? err : null);
            console.log(rec.id);
