...
AbstractClass.include = function(objects, include, cb) {
    ...
    function processIncludeJoin(ij) {
        ...
        if (ij.constructor.name === 'Object') {
            const newIj = [];
            Object.keys(ij).forEach(function(key) {
                const obj = {};
                obj[key] = ij[key];
                newIj.push(obj);
            });
...
function processIncludeItem(cls, objs, include, keyVals, objsByKeys) {
    ...
    const inValues = [];
    for (let j = 0; j < keyVals[relation.keyFrom].length; j++) {
    ...
    function done(err, objsIncluded, cb) {
        ...
        for (let i = 0; i < objsIncluded.length; i++) {
            ...
            for (let j = 0; j < objectsFrom.length; j++) {
        ...
        Object.keys(keysToBeProcessed).forEach(function(key) {
            objectsFrom = objsByKeys[relation.keyFrom][key];
            for (let k = 0; k < objectsFrom.length; k++) {
