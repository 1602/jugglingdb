    ...
    if (!onlySchema) {
        ...
        if (cachedRelations === true && this.__cachedRelations) {
            ...
            Object.keys(relations).forEach(function (attr) {
                if (!data.hasOwnProperty(attr)) {
                    var rel = relations[attr];
                    if(rel instanceof Array) {
                        var relCopy = [];
                        for(var i = 0; i < rel.length; i++) {
                            relCopy.push(rel[i].toObject(onlySchema, cachedRelations));
                        }
                        data[attr] = relCopy;
                    } else {
                        data[attr] = rel.toObject(onlySchema, cachedRelations);
                    }
                }
            });
        }
...
function wrapWithPromise(fn, isClassMethod) {
    return function promisedWrap() {
        ...
        return connection(Model.schema).then(function() {
