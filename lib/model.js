    ...
    function create() {
        obj.trigger('create', function(createDone) {
            obj.trigger('save', function(saveDone) {
                ...
                this.schema.callAdapter('create', modelName, this.constructor._forDB(obj.toObject(true)), function (err, id, rev) {
                    if (id) {
                        obj.__data.id = id;
                        obj.__dataWas.id = id;
...
AbstractClass.upsert = AbstractClass.updateOrCreate = function upsert(data, callback) {
    ...
    if (this.schema.adapter.updateOrCreate) {
        ...
        this.schema.callAdapter('updateOrCreate', Model.modelName, Model._forDB(inst.toObject(true)), function (err, data) {
            var obj;
            if (data) {
...
AbstractClass.count = function (where, cb) {
    ...
    this.schema.callAdapter('count', this.modelName, this._forDB(where), cb);
};
    ...
    function save() {
        inst.trigger('save', function(saveDone) {
            inst.trigger('update', function(updateDone) {
                inst.schema.callAdapter('save', modelName, inst.constructor._forDB(data), function (err) {
                    if (err) {
        ...
        if (!valid) {
            ...
            inst.trigger('save', function (saveDone) {
                inst.trigger('update', function (done) {
                    ...
                    inst.schema.callAdapter('updateAttributes', modelName, inst.id, inst.constructor._forDB(inst.toObject(true)), function (err) {
                        if (!err) {
                            // update _was attrs
                            Object.keys(data).forEach(function (key) {
