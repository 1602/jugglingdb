...
AbstractClass.update = function (params, cb) {
    ...
    this.schema.adapter.update(this.tableName, params, cb);
};
    ...
    function create() {
        obj.trigger('create', function(createDone) {
            obj.trigger('save', function(saveDone) {
                ...
                this._adapter().create(tableName, this.constructor._forDB(obj.toObject(true)), function (err, id, rev) {
                    if (id) {
                        obj.__data.id = id;
                        obj.__dataWas.id = id;
...
AbstractClass.upsert = AbstractClass.updateOrCreate = function upsert(data, callback) {
    ...
    if (this.schema.adapter.updateOrCreate) {
        ...
        this.schema.adapter.updateOrCreate(Model.tableName, inst.toObject(true), function (err, data) {
            var obj;
            if (data) {
...
AbstractClass.exists = function exists(id, cb) {
    ...
    if (id) {
        this.schema.adapter.exists(this.tableName, id, cb);
    } else {
...
AbstractClass.find = function find(id, cb) {
    ...
    this.schema.adapter.find(this.tableName, id, function (err, data) {
        var obj = null;
        if (data) {
            if (!data.id) {
                data.id = id;
...
AbstractClass.all = function all(params, cb) {
    ...
    var constr = this;
    this.schema.adapter.all(this.tableName, params, function (err, data) {
...
AbstractClass.destroyAll = function destroyAll(cb) {
    ...
    this.schema.adapter.destroyAll(this.tableName, function (err) {
...
AbstractClass.count = function (where, cb) {
    ...
    this.schema.adapter.count(this.tableName, cb, where);
};
    ...
    function save() {
        inst.trigger('save', function (saveDone) {
            inst.trigger('update', function (updateDone) {
                inst._adapter().save(tableName, inst.constructor._forDB(data), function (err) {
                    if (err) {
...
AbstractClass.prototype.destroy = function (cb) {
    ...
    this.trigger('destroy', function (destroyed) {
        this._adapter().destroy(this.constructor.tableName, this.id, function (err) {
            if (err) {
                return cb(err);
            }
        ...
        if (!valid) {
            ...
            inst.trigger('save', function (saveDone) {
                inst.trigger('update', function (done) {
                    ...
                    inst._adapter().updateAttributes(tableName, inst.id, inst.constructor._forDB(inst.toObject(true)), function (err) {
                        if (!err) {
                            // update _was attrs
                            Object.keys(data).forEach(function (key) {
