...
AbstractClass.injectRelations = function (objects, filter, cb) {
    if (filter && filter.include) {
        this.include(objects, filter.include, cb);
    } else if (filter && filter.populate) {
        this.populate(objects, filter.populate, cb);
    } else {
        cb(null, objects);
    }
};
