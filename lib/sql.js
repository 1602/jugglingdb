...
BaseSQL.prototype.save = function (model, data, callback) {
    var sql = 'UPDATE ' + this.tableEscaped(model) + ' SET ' + this.toFields(model, data) + ' WHERE ' + this.escapeName('id') + ' = ' + (this.schema.settings.slave ? ('"' + data.id + '"') : Number(data.id));
...
BaseSQL.prototype.find = function find(model, id, callback) {
    ...
    if (isNaN(idNumber)) {
        return callback(new Error('id is not a number'));
    }
    var sql = 'SELECT * FROM ' +
...
BaseSQL.prototype.count = function count(model, callback, where) {
    ...
    var query = 'SELECT count(*) as cnt FROM ' + this.tableEscaped(model);
    if (where) query += ' WHERE ' + buildWhere(where);
