...
/**
 * To digest one or two errors of validation
 */
Errors.prototype.digest = function digest(limit) {
    var errors = []
    limit = limit || 3
    for (var k in this) {
        if (!this.hasOwnProperty(k)) continue;
        limit -= 1;
        errors.push(k + ' ' + this[k])
        if (limit <= 0) break;
    }
    return errors.join('; ')
};
...
function ValidationError(obj) {
    ...
    this.message = obj.errors && obj.errors.digest() || 'Validation error';
